...
static bool
try_lookup_component_owned_resource_transfer_type(
...
init_component_public_string_result(
    WASMComponentInstance *inst,
    const WASMComponentCanonLiftValueInfo *type_info, const uint8 *payload,
    uint32 payload_len, wasm_component_value_t *value_out);

static bool
init_component_public_list_scalar_result(
    WASMComponentInstance *inst,
    const WASMComponentCanonLiftValueInfo *type_info, uint8 *payload,
    uint32 byte_count, wasm_component_value_t *value_out);
...
write_component_public_composite_result_to_memory(
    WASMComponentInstance *inst, const WASMComponent *component,
    const WASMComponentValueType *value_type, uint32 result_index,
    const uint8 *data, uint32 byte_size, uint32 *data_offset_io,
    WASMModuleInstanceCommon *caller_module_inst, uint32 result_area_ptr,
...
convert_utf16_bytes_to_utf8(const uint8 *utf16_bytes, uint32 utf16_byte_len,
                            uint8 **out_utf8, uint32 *out_utf8_len,
                            WASMComponentInstance *inst);
...
try_lookup_component_borrowed_resource_param_type(
    ...
    const WASMComponent *component, const WASMComponentValueType *value_type,
    const char *position, uint32 index,
    bool *is_borrowed_resource_out, WASMComponentCanonLiftValueInfo *type_info_out,
    uint32 *resource_type_idx_out)
{
    ...
    *is_borrowed_resource_out = false;
    ...
    if (value_type->type != WASM_COMP_VAL_TYPE_IDX)
        return true;
    ...
    if (def_type->tag != WASM_COMP_DEF_VAL_BORROW)
        return true;

    if (!resource_state)
        return set_component_call_error(
            inst, "component borrowed resource transfer requires resource state");

    resource_type = wasm_component_resource_lookup_runtime_type_const(
        resource_state, def_type->def_val.borrow->type_idx);
    if (!resource_type)
        return set_component_call_error_fmt(
            inst,
            "component canon lift function %s %u uses unresolved resource type %u",
            position, index, def_type->def_val.borrow->type_idx);

    memset(type_info_out, 0, sizeof(*type_info_out));
    type_info_out->kind = WASM_COMP_CANON_LIFT_VALUE_SCALAR;
    type_info_out->declared_as_defined = true;
    type_info_out->prim_type = WASM_COMP_PRIMVAL_U32;
    type_info_out->core_type = VALUE_TYPE_I32;
    type_info_out->public_kind = WASM_I32;
    *is_borrowed_resource_out = true;
    if (resource_type_idx_out)
        *resource_type_idx_out = def_type->def_val.borrow->type_idx;
    return true;
}
...
destroy_component_resource_state_for_instance(WASMComponentInstance *inst)
{
    ...
    for (i = 0; i < resource_state->owned_handle_count; i++) {
        ...
        else if (canonical_type->kind == WASM_COMP_RUNTIME_RESOURCE_TYPE_IMPORTED
                 && canonical_type->imported_drop_callback) {
            char error_buf[128] = { 0 };
            (void)canonical_type->imported_drop_callback(
                entry->data, canonical_type->imported_drop_user_data, error_buf,
    ...
    switch (sort_idx->sort->sort) {
        ...
        case WASM_COMP_SORT_TYPE:
            if (!inst->resource_state)
                return set_component_runtime_error_fmt(
                    error_buf, error_buf_size,
                    "component type index %u could not resolve resource state",
                    sort_idx->idx);
            out_ref->of.resource_type =
                wasm_component_resource_lookup_runtime_type_const(
                    inst->resource_state, sort_idx->idx);
            if (!out_ref->of.resource_type)
                return set_component_runtime_error_fmt(
                    error_buf, error_buf_size,
                    "component type index %u is out of bounds", sort_idx->idx);
            out_ref->type = WASM_COMP_RUNTIME_REF_RESOURCE_TYPE;
            return true;
        default:
            return set_component_runtime_error_fmt(
                                       ...
                                       wasm_component_export_t *export_type)
{
    ...
    switch (ref.type) {
        ...
        case WASM_COMP_RUNTIME_REF_RESOURCE_TYPE:
            ...
            export_type->value.resource_type = ref.of.resource_type;
            return true;
        default:
            return false;
    }
...
store_component_canon_memory_scalar_value(WASMComponentInstance *inst,
                                          ...
                                          uint8 *out_bytes, uint32 out_size);
    ...
    const WASMFuncType *expected_type, char *error_buf, uint32 error_buf_size)
{
    ...
    if (expected_result_count > 1) {
        ...
        if (core_param_index + 1 != expected_type->param_count
            ...
            return set_component_runtime_error_fmt(
                error_buf, error_buf_size,
                "core import multi-result signature does not match the lowered "
                "component function signature");

        (void)result_vector_size;
        (void)result_vector_align;
        needs_memory = true;
        needs_string = needs_string || has_string_leaf || has_list_string_leaf;
        uses_multi_result_area = true;
    }
...
validate_lowered_result_done:
    ...
    if (needs_memory && !has_memory_opt)
        return set_component_runtime_error_fmt(
            error_buf, error_buf_size,
            uses_multi_result_area
                ? "component canon lower direct core-call bindings require "
                  "memory for multi-result Canonical ABI"
            : needs_string
                ? "component canon lower direct core-call bindings require "
                                      ...
                                      uint64 *raw_args)
{
    ...
    if (!canonical_resource_type
        || (canonical_resource_type->kind != WASM_COMP_RUNTIME_RESOURCE_TYPE_LOCAL
            && !(resource_function->canon_tag == WASM_COMP_CANON_RESOURCE_DROP
                 && canonical_resource_type->kind
            ...
            if (canonical_resource_type->kind
                ...
                if (was_owned) {
                    ...
                    if (!canonical_resource_type->imported_drop_callback) {
                        wasm_runtime_set_exception(
                            caller_module_inst,
                            "component imported resource drop requires a bound "
                            "host destructor");
                        return;
                    }
                    if (!canonical_resource_type->imported_drop_callback(
                            entry->data,
                            canonical_resource_type->imported_drop_user_data,
                ...
                if (was_owned
                    && mutable_canonical_type->handle_table.owned_handle_count > 0)
                    mutable_canonical_type->handle_table.owned_handle_count--;
                return;
            }
            if (!entry->is_owned) {
...
typedef struct LoweredImportMultiResultDecoded {
    wasm_val_t scalar;
    const uint8 *result_bytes;
    uint32 byte_count;
    uint32 result_length;
    uint32 table_byte_count;
    uint32 string_payload_bytes;
} LoweredImportMultiResultDecoded;

static bool
materialize_lowered_import_multi_results(
    WASMComponentInstance *component_inst, const WASMComponent *component,
    ...
    WASMModuleInstanceCommon *caller_module_inst, uint32 result_area_ptr)
{
    ...
    if (result_count > 16) {
        type_infos = wasm_runtime_malloc(sizeof(*type_infos) * result_count);
        ...
        wasm_runtime_set_exception(
            ...
            component_exception
                ...
                : "component lowered core-call trampoline could not resolve the "
                  "multi-result Canonical ABI layout");
        wasm_runtime_clear_exception((WASMModuleInstanceCommon *)component_inst);
        goto cleanup;
    }
    (void)has_string_leaf;
    (void)has_list_scalar_leaf;
    (void)has_list_string_leaf;

    if ((result_area_ptr & (result_vector_align - 1)) != 0
        || !wasm_runtime_validate_app_addr(caller_module_inst, result_area_ptr,
                                           result_vector_size)) {
        wasm_runtime_set_exception(caller_module_inst, "out of bounds memory access");
        goto cleanup;
    }
    ...
    for (uint32 i = 0; i < result_count; i++) {
        ...
        wasm_runtime_clear_exception((WASMModuleInstanceCommon *)component_inst);
        if (!resolve_component_canon_lift_value_shape(
                component, &component_type->results->results[i], "result", i, &shape,
                component_inst)) {
            ...
            wasm_runtime_set_exception(
                ...
                component_exception
                    ? component_exception
                    : "component lowered core-call trampoline could not resolve "
                      "the component result type");
            wasm_runtime_clear_exception((WASMModuleInstanceCommon *)component_inst);
            goto cleanup;
        }
        ...
        switch (type_infos[i].kind) {
            ...
            case WASM_COMP_CANON_LIFT_VALUE_STRING:
                ...
                decoded[i].byte_count = decoded[i].result_length;
                break;

            case WASM_COMP_CANON_LIFT_VALUE_LIST_SCALAR: {
                uint32 element_size =
                    component_scalar_prim_byte_size(type_infos[i].prim_type);

                decoded[i].byte_count = results[i].byte_size;
                if (element_size == 0
                    || (decoded[i].byte_count % element_size) != 0) {
                    wasm_runtime_set_exception(
                        caller_module_inst,
                ...
                decoded[i].result_length = decoded[i].byte_count / element_size;
                decoded[i].result_bytes =
                    wasm_component_value_get_data(&results[i]);
                if (decoded[i].byte_count > 0 && !decoded[i].result_bytes) {
                    wasm_runtime_set_exception(
                        caller_module_inst,
                        "component lowered core-call trampoline could not access "
                        "the list<scalar> result bytes");
                    goto cleanup;
                }
                break;
            }

            case WASM_COMP_CANON_LIFT_VALUE_LIST_STRING: {
                const uint8 *cursor;
                const uint8 *end;
                ...
                cursor = decoded[i].result_bytes;
                end = decoded[i].result_bytes + decoded[i].byte_count;
                status =
                    bh_leb_read(cursor, end, 32, false, &decoded_count, &count_len);
                if (status != BH_LEB_READ_SUCCESS
                    || decoded_count != decoded[i].result_length
                    || count_len > decoded[i].byte_count
                    || !compute_list_scalar_byte_count(decoded[i].result_length, 8,
                                                       &decoded[i].table_byte_count)) {
                    wasm_runtime_set_exception(
                        caller_module_inst,
                ...
                for (uint32 element_index = 0;
                     element_index < decoded[i].result_length; element_index++) {
                    uint64 string_len = 0;
                    size_t len_len = 0;

                    status = bh_leb_read(cursor, end, 32, false, &string_len,
                                         &len_len);
                    if (status != BH_LEB_READ_SUCCESS || string_len > UINT32_MAX
                        || len_len > (size_t)(end - cursor)
                        || string_len > (uint64)(end - cursor - len_len)
                        || decoded[i].string_payload_bytes
                               > UINT32_MAX - (uint32)string_len) {
                        wasm_runtime_set_exception(
                            caller_module_inst,
                            "component lowered core-call trampoline list<string> "
                            "result is malformed");
                    ...
                    decoded[i].string_payload_bytes += (uint32)string_len;
                }
                break;
            }
            ...
            default:
                wasm_runtime_set_exception(
                    caller_module_inst,
                    "component lowered core-call trampoline currently supports "
                    "only scalar, string, list<scalar>, list<string>, or "
                    "tuple/record multi-results");
                goto cleanup;
        }
    }

    payload_cursor = result_area_ptr + result_vector_size;
    for (uint32 i = 0; i < result_count; i++) {
            ...
            if (!resolve_component_canon_lift_value_shape(
                ...
                wasm_runtime_set_exception(
                    ...
                    component_exception
                        ? component_exception
                        : "component lowered core-call trampoline could not "
                          "resolve the component result type");
                wasm_runtime_clear_exception(
                    (WASMModuleInstanceCommon *)component_inst);
                goto cleanup;
            }
            ...
            if (!shape.is_primitive && shape.def_type
                && (shape.def_type->tag == WASM_COMP_DEF_VAL_RECORD
                    || shape.def_type->tag == WASM_COMP_DEF_VAL_TUPLE)) {
                const WASMComponentValueType *result_type =
                    &component_type->results->results[i];
                const uint8 *data = wasm_component_value_get_data(&results[i]);
                uint8 *result_area_bytes = wasm_runtime_addr_app_to_native(
                    caller_module_inst, result_area_ptr);
                uint32 data_offset = 0;
                uint32 payload_offset = payload_cursor - result_area_ptr;

                if (results[i].type.kind != WASM_COMPONENT_VALUE_TYPE_DEFINED) {
                    wasm_runtime_set_exception(
                        caller_module_inst,
                        "component lowered core-call trampoline expected a "
                        "defined composite multi-result");
                    goto cleanup;
                }
                if (results[i].byte_size > 0 && !data) {
                    wasm_runtime_set_exception(
                        caller_module_inst,
                        "component lowered core-call trampoline composite "
                        "multi-result is missing backing bytes");
                    goto cleanup;
                }

                memset(result_slot, 0, result_sizes[i]);
                if (!write_component_public_composite_result_to_memory(
                        component_inst, component, result_type, i, data,
                        results[i].byte_size, &data_offset, caller_module_inst,
                        result_area_ptr, result_area_bytes, result_vector_size,
                        result_offsets[i], &payload_offset)) {
                    const char *component_exception = wasm_runtime_get_exception(
                        (WASMModuleInstanceCommon *)component_inst);
                ...
                if (data_offset != results[i].byte_size) {
                    wasm_runtime_set_exception(
                        caller_module_inst,
                        "component lowered core-call trampoline composite "
                        "multi-result contains trailing bytes");
                    goto cleanup;
                }

                payload_cursor = result_area_ptr + payload_offset;
                continue;
            }
        ...
        switch (type_infos[i].kind) {
            ...
            case WASM_COMP_CANON_LIFT_VALUE_STRING:
            case WASM_COMP_CANON_LIFT_VALUE_LIST_SCALAR:
                if (decoded[i].byte_count > 0) {
                    uint8 *payload_bytes;

                    if (payload_cursor > UINT32_MAX - decoded[i].byte_count
                        || !wasm_runtime_validate_app_addr(
                            caller_module_inst, payload_cursor,
                            decoded[i].byte_count)) {
                        wasm_runtime_set_exception(caller_module_inst,
                                                   "out of bounds memory access");
                        goto cleanup;
                    }

                    payload_bytes = wasm_runtime_addr_app_to_native(
                        caller_module_inst, payload_cursor);
                    if (!payload_bytes) {
                        wasm_runtime_set_exception(caller_module_inst,
                                                   "out of bounds memory access");
                    ...
                    memcpy(payload_bytes, decoded[i].result_bytes,
                           decoded[i].byte_count);
                    payload_ptr = payload_cursor;
                    payload_cursor += decoded[i].byte_count;
                }

                bh_memcpy_s(result_slot, sizeof(uint32), &payload_ptr,
                            sizeof(uint32));
                bh_memcpy_s(result_slot + sizeof(uint32), sizeof(uint32),
                            &decoded[i].result_length, sizeof(uint32));
                break;

            case WASM_COMP_CANON_LIFT_VALUE_LIST_STRING:
                if (decoded[i].table_byte_count > 0
                    || decoded[i].string_payload_bytes > 0) {
                    const uint8 *cursor = decoded[i].result_bytes;
                    const uint8 *end = decoded[i].result_bytes + decoded[i].byte_count;
                    uint8 *payload_bytes;
                    uint32 string_cursor;
                    uint64 ignored_count = 0;
                    size_t count_len = 0;

                    if (payload_cursor > UINT32_MAX - decoded[i].table_byte_count
                        || payload_cursor + decoded[i].table_byte_count
                               > UINT32_MAX - decoded[i].string_payload_bytes
                        || !wasm_runtime_validate_app_addr(
                            caller_module_inst, payload_cursor,
                            decoded[i].table_byte_count
                                + decoded[i].string_payload_bytes)) {
                    ...
                    (void)bh_leb_read(cursor, end, 32, false, &ignored_count,
                                      &count_len);
                    cursor += count_len;
                    string_cursor = payload_cursor + decoded[i].table_byte_count;
                    payload_ptr = payload_cursor;
                    for (uint32 element_index = 0;
                         element_index < decoded[i].result_length; element_index++) {
                        uint64 string_len = 0;
...
cleanup:
    ...
    if (decoded != stack_decoded)
        wasm_runtime_free(decoded);
    if (result_offsets != stack_offsets)
...
#define FREE_LOWERED_ARG_TRACKING()                                             \
    ...
    if (expected_result_count > 1) {
        ...
        if (!result_shape.is_primitive && result_shape.def_type
            ...
            if (!try_lookup_component_owned_resource_transfer_type(
                    ...
                    &has_owned_resource_result, &result_type_info, NULL)
                || (!has_owned_resource_result
                    && !try_lookup_component_borrowed_resource_param_type(
                        component_inst, lowered_function->resource_state, component,
                        component_type->results->results, "result", 0,
                        &has_borrowed_resource_result, &result_type_info,
                        &borrowed_result_resource_type_idx))
                || (!has_owned_resource_result
                    && !has_borrowed_resource_result
                    && !lookup_component_canon_lift_value_type(
                        component, component_type->results->results, "result", 0,
                        true, false, true, true, &result_type_info,
                        component_inst))) {
                const char *component_exception = wasm_runtime_get_exception(
                    (WASMModuleInstanceCommon *)component_inst);
                if (call_args != stack_args)
            ...
            if (!try_lookup_component_owned_resource_transfer_type(
                ...
                || (!has_owned_resource_param && !has_borrowed_resource_param
                    && !lookup_component_canon_lift_value_type(
                        ...
                        "parameter", i, true, true, true, false, &type_info,
                        component_inst))) {
                const char *component_exception =
                    wasm_runtime_get_exception((WASMModuleInstanceCommon *)component_inst);
                if (call_args != stack_args)
    ...
    if (expected_result_count > 1) {
        ...
        if (func_type->result_count != 0 || core_param_index >= func_type->param_count) {
            ...
            wasm_runtime_set_exception(
                caller_module_inst,
                "component lowered core-call trampoline multi-results must use "
                "an i32 return-area pointer parameter");
            return;
        }
        ...
        case WASM_COMP_DEF_VAL_RESULT:
            if (def_type->def_val.result) {
                if (def_type->def_val.result->result_type) {
                    if (!classify_component_runtime_composite_param(
                            component,
                            def_type->def_val.result->result_type,
                            param_index, has_string_leaf_out,
                            has_list_scalar_leaf_out,
                            has_list_string_leaf_out, error_buf,
                            error_buf_size))
                ...
                if (def_type->def_val.result->error_type) {
                    if (!classify_component_runtime_composite_param(
                            component,
                            def_type->def_val.result->error_type,
                            param_index, has_string_leaf_out,
                            has_list_scalar_leaf_out,
                            has_list_string_leaf_out, error_buf,
                            error_buf_size))
            ...
            return true;
        case WASM_COMP_DEF_VAL_OPTION:
            if (def_type->def_val.option
                && def_type->def_val.option->element_type) {
                if (!classify_component_runtime_composite_param(
                        component, def_type->def_val.option->element_type,
                        param_index, has_string_leaf_out,
    ...
    switch (def_type->tag) {
        ...
        case WASM_COMP_DEF_VAL_OPTION:
            if (def_type->def_val.option
                ...
                if (!classify_component_runtime_composite_result(
                        component, def_type->def_val.option->element_type,
                        has_string_leaf_out, has_list_scalar_leaf_out,
                        has_list_string_leaf_out, error_buf, error_buf_size))
                    return false;
            }
            return true;
        default:
...
static bool
try_lookup_component_owned_resource_type(
    WASMComponentInstance *inst,
    const WASMComponentRuntimeResourceState *resource_state,
    const WASMComponent *component, const WASMComponentValueType *value_type,
    const char *position, uint32 index,
    ...
    uint32 *resource_type_idx_out)
{
    ...
    *is_owned_resource_out = false;
    ...
    type_entry = wasm_component_lookup_type(component, value_type->type_specific.type_idx);
    if (!type_entry)
        return set_component_call_error_fmt(
            inst,
            "component canon lift function %s %u uses unresolved type index %u",
    ...
    def_type = type_entry->type.def_val_type;
    if (def_type->tag != WASM_COMP_DEF_VAL_OWN)
        return true;

    if (!resource_state)
        return set_component_call_error(
            inst, "component owned resource result requires resource state");

    resource_type = wasm_component_resource_lookup_runtime_type_const(
        resource_state, def_type->def_val.owned->type_idx);
    if (!resource_type)
        return set_component_call_error_fmt(
            inst,
            "component canon lift function %s %u uses unresolved resource type %u",
            position, index, def_type->def_val.owned->type_idx);

    canonical_type = wasm_component_resource_lookup_runtime_type_const(
        resource_state, resource_type->canonical_type_idx);
    if (!canonical_type
        || canonical_type->kind != WASM_COMP_RUNTIME_RESOURCE_TYPE_LOCAL)
        return set_component_call_error_fmt(
            inst,
            "component canon lift function %s %u only supports own<resource> "
            "results for local resource types and aliases thereof",
            position, index);
    ...
    type_info_out->core_type = VALUE_TYPE_I32;
    type_info_out->public_kind = WASM_I32;
    *is_owned_resource_out = true;
    if (resource_type_idx_out)
        *resource_type_idx_out = def_type->def_val.owned->type_idx;
    return true;
}

static bool
try_lookup_component_droppable_owned_resource_type(
    WASMComponentInstance *inst,
    const WASMComponentRuntimeResourceState *resource_state,
    const WASMComponent *component, const WASMComponentValueType *value_type,
    const char *position, uint32 index,
    ...
    *is_owned_resource_out = false;
    ...
    if (!canonical_type
        ...
        return set_component_call_error_fmt(
            inst,
            "component canon lift function %s %u only supports own<resource> "
            "results for local or imported resource types and aliases thereof",
            position, index);
    ...
    uint32 *resource_type_idx_out)
{
    ...
    *is_owned_resource_out = false;
    ...
    if (!resource_state)
        return set_component_call_error(
            inst, "component owned resource transfer requires resource state");
...
resolve_component_owned_result_type(
    WASMComponentInstance *inst, const WASMComponentRuntimeFunc *function,
    uint32 result_index,
    const WASMComponentRuntimeResourceState **resource_state_out,
    WASMComponentCanonLiftValueInfo *type_info_out,
    uint32 *resource_type_idx_out)
{
    const WASMComponent *component;
    WASMComponentFuncType *component_type = NULL;
    const WASMComponentRuntimeResourceState *resource_state =
        function && function->resource_state ? function->resource_state
                                            : inst->resource_state;
    ...
    if (!resolve_component_func_type(inst, function, "component function",
                                     &component_type))
        return false;

    if (result_index >= get_component_func_result_count(component_type))
        return set_component_call_error_fmt(
    ...
    if (!is_owned_resource)
        return set_component_call_error_fmt(
            inst, "component function result %u is not an own<resource> value",
            result_index);

    if (resource_state_out)
        *resource_state_out = resource_state;
    ...
    uint32 resource_type_idx, uint32 handle, bool *consumed_out)
{
    ...
    *consumed_out = false;
    ...
    if (!inst || !resource_state)
        return set_component_call_error(
            inst, "component owned resource drop requires resource state");

    resource_type = wasm_component_resource_lookup_runtime_type_const(
        resource_state, resource_type_idx);
    if (!resource_type)
        return set_component_call_error_fmt(
            inst, "component type index %u is not a runtime resource type",
            resource_type_idx);
    ...
    if (!canonical_type)
        return set_component_call_error(
            inst, "component owned resource drop requires a canonical resource type");

    if (handle == 0 || handle - 1 >= canonical_type->handle_table.entry_count)
        return set_component_call_error(inst,
                                        "component resource handle is invalid");

    entry = &((WASMComponentRuntimeResourceType *)canonical_type)
                 ->handle_table.entries[handle - 1];
    if (!entry->is_live || !entry->is_owned)
        return set_component_call_error(
    ...
    if (canonical_type->kind == WASM_COMP_RUNTIME_RESOURCE_TYPE_IMPORTED) {
        ...
        if (!canonical_type->imported_drop_callback)
            return set_component_call_error(
                inst,
                "component imported resource drop requires a bound host destructor");
        ...
        if (mutable_canonical_type->handle_table.live_handle_count > 0)
            mutable_canonical_type->handle_table.live_handle_count--;
        if (mutable_canonical_type->handle_table.owned_handle_count > 0)
            mutable_canonical_type->handle_table.owned_handle_count--;
        *consumed_out = true;
        return true;
    }
    ...
    if (canonical_type->kind != WASM_COMP_RUNTIME_RESOURCE_TYPE_LOCAL)
        return set_component_call_error(
            inst, "component owned resource drop only supports local or imported "
                  "resource types and aliases thereof");

    if (canonical_type->has_dtor) {
        wasm_val_t dtor_arg = { 0 };
        WASMComponentCoreRuntimeRef *dtor_ref;
        WASMComponentInstance *resource_owner_inst =
            resource_state->owner_instance ? resource_state->owner_instance : inst;

        if (!resource_owner_inst->core_funcs
            || canonical_type->dtor_func_idx >= resource_owner_inst->core_func_count)
            return set_component_call_error(
                inst, "component owned resource drop could not resolve the "
                      "destructor core function");

        dtor_ref = &resource_owner_inst->core_funcs[canonical_type->dtor_func_idx];
        if (dtor_ref->type != WASM_COMP_CORE_RUNTIME_REF_FUNC
            || !dtor_ref->of.function)
            return set_component_call_error(
                inst, "component owned resource drop could not resolve the "
                      "destructor core function");
        ...
        dtor_arg.of.i32 = (int32)(uint32)(uintptr_t)entry->data;
        if (!call_core_function_from_resource_builtin(
                (WASMModuleInstanceCommon *)inst, dtor_ref,
                "component owned resource drop could not acquire a destructor "
                "execution environment",
...
cleanup_host_component_resource_arguments(WASMComponentInstance *inst,
                                          ...
                                          uint32 arg_count, bool restore_only)
{
    for (uint32 i = 0; i < arg_count; i++) {
        ...
        if (resource_value->kind
            ...
            wasm_component_value_destroy(&args[i]);
            continue;
        }
        ...
        if (!restore_only
            && !drop_component_owned_resource_handle_internal(
                inst, resource_state ? resource_state : inst->resource_state,
                resource_type_idx, handle, &consumed))
            return false;
    }
...
validate_component_host_import_value_type(
    ...
    bool *is_owned_resource_out, bool *is_borrowed_resource_out, char *error_buf,
    uint32 error_buf_size)
{
    const WASMComponentTypes *type_entry;
    WASMComponentDefValType *def_type;
    ...
    *is_composite_out = false;
    ...
    if (def_type->tag == WASM_COMP_DEF_VAL_BORROW)
        return is_borrowed_resource_out ? (*is_borrowed_resource_out = true, true)
                                        : true;
            ...
            case WASM_COMP_DEF_VAL_RECORD:
                ...
                for (uint32 i = 0; i < def_type->def_val.record->count; i++) {
                    ...
                    if (!validate_component_host_import_value_type(
                            ...
                            &nested_is_list_scalar, &nested_is_composite, NULL,
                            NULL,
                            error_buf, error_buf_size))
                        return false;
                                         ...
                                         uint32 error_buf_size)
{
    ...
    function->has_owned_resource_params = false;
    function->has_borrowed_resource_params = false;
    function->has_string_result = false;
    function->has_list_scalar_result = false;
    function->has_composite_result = false;
    function->has_owned_resource_result = false;
    function->has_borrowed_resource_result = false;
    ...
    if (func_type->params) {
        for (i = 0; i < func_type->params->count; i++) {
            ...
            if (!validate_component_host_import_value_type(
                    ...
                    &is_owned_resource, &is_borrowed_resource, error_buf,
                    error_buf_size))
                return false;
            if (is_string)
            ...
            if (is_owned_resource)
                function->has_owned_resource_params = true;
            if (is_borrowed_resource)
                function->has_borrowed_resource_params = true;
        }
    }
    ...
    if (func_type->results && func_type->results->results) {
        if (func_type->results->count > 1) {
            for (i = 0; i < func_type->results->count; i++) {
                ...
                if (!validate_component_host_import_value_type(
                        component, &func_type->results->results[i], import_name,
                        "result", i, &is_string, &is_list_scalar, &is_composite,
                        &is_owned_resource, &is_borrowed_resource, error_buf,
                        error_buf_size))
                    return false;
        ...
        if (!validate_component_host_import_value_type(
                ...
                &function->has_list_scalar_result, &is_composite,
                &is_owned_resource, &is_borrowed_resource, error_buf,
                error_buf_size))
            return false;
        if (is_composite)
    ...
    if (func_type->params) {
        ...
        if (func_type->results->count > 1) {
            for (i = 0; i < func_type->results->count; i++) {
                ...
                bool is_composite_result = false;
                ...
                if (!is_primitive_result
                    && func_type->results->results[i].type == WASM_COMP_VAL_TYPE_IDX) {
                    const WASMComponentTypes *result_type_entry =
                        wasm_component_lookup_type(
                            component,
                            func_type->results->results[i].type_specific.type_idx);
                    ...
                    if (result_type_entry->tag != WASM_COMP_DEF_TYPE
                        ...
                        return set_component_runtime_error_fmt(
                            error_buf, error_buf_size,
                            "component canon lift function uses non-value type "
                            "index %u",
                            func_type->results->results[i].type_specific.type_idx);
                ...
                if (!resolve_component_lift_string_usage(
                        component, &func_type->results->results[i],
                        &is_string_result, error_buf, error_buf_size))
                    return false;
                if (is_string_result) {
                    function->has_string_result = true;
                    continue;
                ...
                if (!resolve_component_lift_list_string_usage(
                        component, &func_type->results->results[i],
                        &is_list_string_result, error_buf, error_buf_size))
                    return false;
                if (is_list_string_result) {
                    function->has_string_result = true;
                    continue;
                ...
                if (!resolve_component_lift_list_scalar_usage(
                        component, &func_type->results->results[i],
                        &is_list_scalar_result, error_buf, error_buf_size))
                    return false;
                if (is_list_scalar_result) {
                    function->has_list_scalar_result = true;
                    continue;
                }
        ...
        bool has_list_string_result = false;
        bool is_primitive_result = false;
        uint8 result_prim_type = 0;
        ...
        if (!is_primitive_result && func_type->results->results->type == WASM_COMP_VAL_TYPE_IDX) {
                ...
                if (composite_has_string || composite_has_list_scalar
                    ...
                    function->memory_result_kind =
                      ...
                      || (!has_string_result
                          && !resolve_component_lift_list_string_usage(
                              component, func_type->results->results,
                              &has_list_string_result, error_buf,
                              error_buf_size))
                      || (!has_string_result && !has_list_string_result
                          && !resolve_component_lift_list_scalar_usage(
                              component, func_type->results->results,
            ...
            if (!has_string_result
                && !resolve_component_lift_list_string_usage(
                    component, func_type->results->results,
                    &has_list_string_result, error_buf, error_buf_size))
                return false;
            if (!has_string_result && !has_list_string_result
                && !resolve_component_lift_list_scalar_usage(
                    component, func_type->results->results,
                    &has_list_scalar_result, error_buf, error_buf_size))
                return false;
        ...
        else if (has_list_string_result)
            function->memory_result_kind =
                WASM_COMP_RUNTIME_CANON_LIFT_MEMORY_RESULT_STRING;
        ...
        case WASM_COMP_DEF_VAL_RESULT:
                ...
                return set_component_composite_param_leaf_error(inst, param_index);
            {
                ...
                if (!decode_component_public_scalar_prefix(
                    ...
                    return false;
                (*offset_io) += consumed;
                if (ignored_discriminant.of.i32 == 0
                ...
                else if (ignored_discriminant.of.i32 == 1
        ...
        case WASM_COMP_DEF_VAL_OPTION:
                ...
                return set_component_composite_param_leaf_error(inst, param_index);
            {
                ...
                if (!decode_component_public_scalar_prefix(
                    ...
                    return false;
                (*offset_io) += consumed;
                if (ignored_discriminant.of.i32 == 1
    ...
    *has_list_string_leaf_out = false;
    ...
    switch (shape.def_type->tag) {
        ...
        case WASM_COMP_DEF_VAL_RESULT:
        {
            ...
            if (shape.def_type->def_val.result) {
                if (shape.def_type->def_val.result->result_type) {
                    if (!compute_component_canon_abi_layout(
                            inst, resource_state, component,
                            shape.def_type->def_val.result->result_type,
                            result_index, &inner_size, &inner_align,
                            &inner_has_string, &inner_has_list_scalar,
                            &inner_has_list_string))
                        return false;
                }
                if (shape.def_type->def_val.result->error_type) {
                    uint32 error_size = 0, error_align = 1;
                    bool error_has_string = false;
                    bool error_has_list_scalar = false;
                    bool error_has_list_string = false;
                    if (!compute_component_canon_abi_layout(
                            inst, resource_state, component,
                            shape.def_type->def_val.result->error_type,
                            result_index, &error_size, &error_align,
                            &error_has_string, &error_has_list_scalar,
                            &error_has_list_string))
                        return false;
                    inner_size = inner_size > error_size ? inner_size : error_size;
                    inner_align = inner_align > error_align ? inner_align : error_align;
                    inner_has_string = inner_has_string || error_has_string;
                    inner_has_list_scalar = inner_has_list_scalar || error_has_list_scalar;
                    inner_has_list_string = inner_has_list_string || error_has_list_string;
                }
            }

            /* Discriminant: 4 bytes, align 4 */
            *size_out = 4 + inner_size;
            *align_out = inner_align > 4 ? inner_align : 4;
            *has_string_leaf_out = inner_has_string;
            *has_list_scalar_leaf_out = inner_has_list_scalar;
    ...
    if (shape.is_primitive) {
        if (shape.prim_type == WASM_COMP_PRIMVAL_STRING) {
            ...
            if (payload_len > 0) {
                ...
                payload =
                    wasm_runtime_addr_app_to_native(caller_module_inst, payload_ptr);
                if (string_encoding == WASM_COMP_RUNTIME_STRING_ENCODING_UTF16
                    && payload_len > 0) {
                    uint8 *utf8_bytes = NULL;
                    uint32 utf8_len = 0;
                    if (!convert_utf16_bytes_to_utf8(
    ...
    uint32 *leaf_count_io)
{
    ...
    switch (shape.def_type->tag) {
        ...
        case WASM_COMP_DEF_VAL_RESULT:
            ...
            /* Recurse into result_type for payload leaves (both ok and
               error cases must produce the same flat count; for the
               scalar+scalar case, result_type is sufficient). */
            if (shape.def_type->def_val.result
                && shape.def_type->def_val.result->result_type) {
                if (!collect_component_composite_result_leaves(
                        inst, component, resource_state,
            ...
            else if (shape.def_type->def_val.result
                     && shape.def_type->def_val.result->error_type) {
                if (!collect_component_composite_result_leaves(
                        inst, component, resource_state,
                        shape.def_type->def_val.result->error_type,
        ...
        case WASM_COMP_DEF_VAL_OPTION:
            ...
            /* Recurse into inner element type for payload leaves */
            if (shape.def_type->def_val.option->element_type) {
                if (!collect_component_composite_result_leaves(
                        inst, component, resource_state,
                        shape.def_type->def_val.option->element_type,
...
static bool
init_component_public_memory_list_string_result(
    WASMComponentInstance *inst, const WASMComponentRuntimeFunc *function,
    uint32 result_area_ptr, const char *result_area_desc,
    wasm_component_value_t *value)
{
    uint8 *ret_area_bytes = NULL;
    uint8 *table_bytes = NULL;
    uint32 table_ptr = 0, element_count = 0, table_byte_count = 0;
    uint32 stack_entries[32];
    uint32 *entries = stack_entries;
    WASMComponentResultPayloadBuilder builder;
    uint8 count_buf[5];
    uint32 count_len;
    bool ok = false;
    ...
    if (!get_component_canon_memory_bytes(inst, function, result_area_ptr, 8,
                                          result_area_desc, &ret_area_bytes))
        return false;

    memcpy(&table_ptr, ret_area_bytes, sizeof(table_ptr));
    memcpy(&element_count, ret_area_bytes + sizeof(table_ptr),
           sizeof(element_count));

    if (element_count > 0) {
        if ((table_ptr & 3) != 0)
            return set_component_call_error(
                inst, "component canon lift function list<string> result table "
                      "is misaligned");
        if (!compute_list_scalar_byte_count(element_count, 8, &table_byte_count))
            return set_component_call_error(
                inst, "component canon lift function list<string> result "
                      "table size overflow");
        if (element_count > sizeof(stack_entries) / sizeof(stack_entries[0]) / 2) {
            entries = wasm_runtime_malloc(sizeof(uint32) * element_count * 2);
            if (!entries)
                return set_component_call_error(
                    inst, "component canon lift function could not allocate "
                          "list<string> result metadata");
        }
        if (!get_component_canon_memory_bytes(inst, function, table_ptr,
                                              table_byte_count,
        ...
        memcpy(entries, table_bytes, table_byte_count);
    }

    init_component_result_payload_builder(&builder);
    count_len = encode_component_unsigned_leb(element_count, count_buf);
    if (!append_component_result_payload_bytes(inst, &builder, count_buf,
                                               count_len)) {
        destroy_component_result_payload_builder(&builder);
        ok = false;
        goto cleanup;
    ...
    for (uint32 i = 0; i < element_count; i++) {
        uint8 *payload_bytes = NULL;
        uint32 payload_ptr = entries[i * 2];
        uint32 payload_len = entries[i * 2 + 1];

        if (payload_len > 0) {
            if (!get_component_canon_memory_bytes(inst, function, payload_ptr,
                                                  payload_len,
                                                  "list<string> result payload",
                                                  &payload_bytes)) {
                destroy_component_result_payload_builder(&builder);
                ok = false;
                goto cleanup;
            ...
            if (!append_component_result_string_leaf(inst, &builder, payload_bytes,
                                                    payload_len)) {
                destroy_component_result_payload_builder(&builder);
                ok = false;
                goto cleanup;
        ...
        else if (!append_component_result_string_leaf(inst, &builder, NULL, 0)) {
            destroy_component_result_payload_builder(&builder);
            ok = false;
            goto cleanup;
        }
...
init_component_public_memory_sequence_result(
    ...
    const char *result_area_desc, wasm_component_value_t *value)
{
    uint8 *ret_area_bytes = NULL;
    uint8 *payload_bytes = NULL;
    uint32 payload_ptr = 0, payload_len = 0;
    ...
    if (!result_info || !value
        ...
        return set_component_call_error(
            inst, "component canon lift function result uses unsupported "
                  "memory-backed result shape");

    if (result_info->kind == WASM_COMP_CANON_LIFT_VALUE_LIST_STRING)
        return init_component_public_memory_list_string_result(
            inst, function, result_area_ptr, result_area_desc, value);

    result_desc = result_info->kind == WASM_COMP_CANON_LIFT_VALUE_STRING
                      ? "string"
                      : "list<scalar>";
    ...
    if (result_info->kind == WASM_COMP_CANON_LIFT_VALUE_STRING) {
        if (!init_component_public_string_result(inst, result_info, payload_copy,
                                                 byte_count, value)) {
            if (payload_copy)
                wasm_runtime_free(payload_copy);
            return false;
                 ...
                 inst, result_info, payload_copy, byte_count, value)) {
        if (payload_copy)
            wasm_runtime_free(payload_copy);
        return false;
    }

    if (result_info->kind == WASM_COMP_CANON_LIFT_VALUE_LIST_SCALAR)
        payload_copy = NULL;
    if (payload_copy)
        wasm_runtime_free(payload_copy);
    return true;
    ...
    const WASMComponentRuntimeResourceState *resource_state_override)
{
    ...
    bool multi_result_retptr_vector = false;
    bool multi_result_retptr_has_string = false;
    bool multi_result_retptr_has_list_scalar = false;
    uint32 borrowed_result_resource_type_idx =
        WASM_COMPONENT_RESOURCE_INVALID_TYPE_IDX;
    uint32 memory_result_retptr = 0;
    uint32 result_vector_size = 0, result_vector_align = 1;
    WASMComponentRuntimeStringEncoding effective_string_encoding;
    ...
    wasm_runtime_clear_exception((WASMModuleInstanceCommon *)inst);
    effective_string_encoding = function->string_encoding;
    if (effective_string_encoding == WASM_COMP_RUNTIME_STRING_ENCODING_NONE
        && function->canon_opts) {
        for (i = 0; i < function->canon_opts->canon_opts_count; i++) {
            if (function->canon_opts->canon_opts[i].tag
                == WASM_COMP_CANON_OPT_STRING_UTF8) {
                effective_string_encoding =
                    WASM_COMP_RUNTIME_STRING_ENCODING_UTF8;
                break;
            }
        }
    ...
    if (function->kind == WASM_COMP_RUNTIME_FUNC_HOST_IMPORT) {
        ...
        if (!function->host_callback)
            goto host_import_param_fail_missing_callback;
        ...
        for (uint32 result_i = 0; result_i < expected_result_count; result_i++) {
            WASMComponentCanonLiftValueShape callback_result_shape = { 0 };
            bool result_is_composite = false;

            if (!resolve_component_canon_lift_value_shape(
                    component, &component_type->results->results[result_i], "result",
                    result_i, &callback_result_shape, inst))
                goto host_import_success_fail;

            result_is_composite =
                !callback_result_shape.is_primitive && callback_result_shape.def_type
                && (callback_result_shape.def_type->tag == WASM_COMP_DEF_VAL_RECORD
                    || callback_result_shape.def_type->tag == WASM_COMP_DEF_VAL_TUPLE);
            ...
            if (result_is_composite) {
                ...
                bool host_owned_resource_result = false;
...
host_import_param_fail_missing_callback:
        set_component_call_error(inst,
                                 "host component function callback is missing");
host_import_param_fail:
        if (!cleanup_host_component_resource_arguments(inst, callback_args,
                                                      host_param_count, true))
            goto host_import_cleanup_fail;
        if (callback_args != stack_callback_args)
...
host_import_cleanup_fail:
    ...
    if (expected_result_count > 1) {
        if (core_type->result_count == expected_result_count) {
            ...
            bool only_supported_retptr_results = true;
            ...
            if (!have_retptr_memory || !have_retptr_post_return)
                return set_component_call_error(
                    inst, "component canon lift function only supports direct "
                          "scalar multi-value or retptr-backed supported "
                          "multi-result signatures");
            ...
            for (i = 0; i < expected_result_count; i++) {
                ...
                if (result_info.kind != WASM_COMP_CANON_LIFT_VALUE_SCALAR
                    ...
                    && result_info.kind != WASM_COMP_CANON_LIFT_VALUE_LIST_SCALAR
                    && result_info.kind != WASM_COMP_CANON_LIFT_VALUE_LIST_STRING) {
                    only_supported_retptr_results = false;
                    break;
                }
            }

            if (!only_supported_retptr_results)
                return set_component_call_error(
                    inst, "component canon lift function only supports "
                          "retptr-backed scalar, string, list<scalar>, "
                          "list<string>, and tuple/record "
                          "multi-result signatures");

            multi_result_retptr_has_string =
                has_string_leaf || has_list_string_leaf;
            multi_result_retptr_has_list_scalar = has_list_scalar_leaf;
            multi_result_retptr_vector = true;
        }
        else
            return set_component_call_error(
                inst, "component canon lift function only supports direct "
                      "scalar multi-value or retptr-backed supported multi-result "
                      "signatures");
    }
    ...
    if (!multi_result_scalar_only && expected_result_count == 1) {
        ...
        if (has_composite_result) {
            ...
            if (!try_lookup_component_owned_resource_type(
                    inst, function->resource_state ? function->resource_state
                                                   : inst->resource_state,
                    component, component_type->results->results, "result", 0,
                    &has_owned_resource_result, &result_info, NULL))
                goto cleanup;

            if (!has_owned_resource_result
                && !try_lookup_component_borrowed_resource_param_type(
                    inst, function->resource_state ? function->resource_state
                                                   : inst->resource_state,
                    component, component_type->results->results, "result", 0,
    ...
    else if (!multi_result_scalar_only && !multi_result_retptr_vector
    ...
    if (multi_result_scalar_only) {
        ...
        if ((memory_result_retptr & (result_vector_align - 1)) != 0) {
            set_component_call_error(
                inst, "component canon lift function multi-result retptr "
                      "returned a misaligned result area");
            call_succeeded = false;
            goto cleanup;
        }
        ...
        for (i = 0; i < expected_result_count; i++) {
            ...
            if (!result_shape_local.is_primitive && result_shape_local.def_type
                && (result_shape_local.def_type->tag == WASM_COMP_DEF_VAL_RECORD
                    || result_shape_local.def_type->tag
                           == WASM_COMP_DEF_VAL_TUPLE)) {
                WASMComponentResultPayloadBuilder builder;
                ...
                if (!decode_component_canon_composite_result_value(
                        inst, function, component,
                        &component_type->results->results[i], i, ret_area_bytes,
                        result_vector_size, result_offsets[i], &builder)
                    || !finish_component_result_payload(&results[i], &builder)) {
                    destroy_component_result_payload_builder(&builder);
                    for (uint32 j = 0; j < i; j++)
                        wasm_component_value_destroy(&results[j]);
                    if (retptr_owned_result_handles
            ...
            else if (!init_component_public_memory_sequence_result(
                         inst, function, &result_info_local,
                         memory_result_retptr + result_offsets[i],
        ...
        if (has_composite_result) {
            ...
            if (has_borrowed_resource_result) {
                ...
                if (core_results[0].kind != WASM_I32) {
                    set_component_call_error(
                        inst, "component canon lift function borrowed result "
                              "returned an unexpected result kind");
                    call_succeeded = false;
                    goto cleanup;
                }
                ...
                if (!borrowed_param
                    ...
                    if (!wasm_runtime_get_exception((WASMModuleInstanceCommon *)inst))
                        set_component_call_error(
                            inst, "component canon lift function borrowed result "
                                  "must alias a current borrowed parameter");
                    call_succeeded = false;
                    goto cleanup;
                }
            }
            ...
            if (core_results[0].kind != WASM_I32) {
                set_component_call_error(
                    ...
                    result_info.kind == WASM_COMP_CANON_LIFT_VALUE_STRING
                        ...
                        : result_info.kind
                                  == WASM_COMP_CANON_LIFT_VALUE_LIST_STRING
                              ? "component canon lift function list<string> "
                                "result returned an unexpected result kind"
            ...
            memory_result_retptr = (uint32)core_results[0].of.i32;
            have_memory_result_ptr = true;
            if (!init_component_public_memory_sequence_result(
                    inst, function, &result_info, memory_result_retptr,
                    result_info.kind == WASM_COMP_CANON_LIFT_VALUE_STRING
                        ? "string result area"
                        : result_info.kind
                                  == WASM_COMP_CANON_LIFT_VALUE_LIST_STRING
                              ? "list<string> result area"
...
wasm_component_drop_owned_result(WASMComponentInstance *inst,
                                 ...
                                 wasm_component_value_t *value)
{
    const WASMComponentRuntimeResourceState *resource_state = NULL;
    WASMComponentCanonLiftValueInfo type_info;
    wasm_val_t handle_value = { 0 };
    uint32 resource_type_idx = WASM_COMPONENT_RESOURCE_INVALID_TYPE_IDX;
    bool consumed = false;
    ...
    if (!function)
        return set_component_call_error(inst, "component function handle is null");

    if (!value)
        return set_component_call_error(inst, "component owned resource value is null");

    if (value->storage_kind == WASM_COMPONENT_VALUE_STORAGE_NONE
        && value->byte_size == 0)
        return set_component_call_error(
            inst,
            "component owned resource result has already been consumed");

    if (!resolve_component_owned_result_type(inst, function, result_index,
                                             &resource_state,
                                             &type_info, &resource_type_idx))
        return false;

    if (!decode_component_public_scalar_value(inst, value, &type_info, "result",
                                              result_index, &handle_value))
        return false;

    drop_ok = drop_component_owned_resource_handle_internal(
        inst, resource_state, resource_type_idx, (uint32)handle_value.of.i32,
        &consumed);
    if (consumed)
        wasm_component_value_destroy(value);
    return drop_ok;
    ...
    for (j = 0; j < export_section->count; j++) {
        ...
        switch (component_export->sort_idx->sort->sort) {
            ...
            case WASM_COMP_SORT_COMPONENT:
                ...
                break;
            case WASM_COMP_SORT_TYPE:
                expected_type = WASM_COMP_RUNTIME_REF_RESOURCE_TYPE;
                sort_name = "resource type";
                break;
            default:
                return set_component_runtime_error_fmt(
                    error_buf, error_buf_size,
        ...
        if (component_export->sort_idx->sort->sort == WASM_COMP_SORT_TYPE) {
            if (!runtime_inst->resource_state)
                return set_component_runtime_error_fmt(
                    error_buf, error_buf_size,
                    "nested component export \"%s\" could not resolve resource "
                    "state",
                    get_component_export_name(component_export->export_name));
            memset(&ref, 0, sizeof(ref));
            ref.type = WASM_COMP_RUNTIME_REF_RESOURCE_TYPE;
            ref.of.resource_type = wasm_component_resource_lookup_runtime_type_const(
                runtime_inst->resource_state, component_export->sort_idx->idx);
            if (!ref.of.resource_type)
                return set_component_runtime_error_fmt(
                    error_buf, error_buf_size,
                    "nested component export \"%s\" type index %u is out of bounds",
                    get_component_export_name(component_export->export_name),
                    component_export->sort_idx->idx);
        }
        else if (!resolve_nested_component_local_sort_idx(
...
static bool
validate_component_resource_type_bound_supported(
    const WASMComponentTypeBound *type_bound, const char *import_name,
                ...
                switch (export_decl->extern_desc->type) {
                    case WASM_COMP_EXTERN_COMPONENT:
                        actual_export = lookup_component_export_by_name(
                            actual_component, expected_export_name,
                            WASM_COMP_SORT_COMPONENT);
                        if (!actual_export)
                            return member_name
                                       ? set_component_runtime_error_fmt(
                                             error_buf, error_buf_size,
                                             "component import \"%s\" instance "
                                             "export \"%s\" expects component "
                                             "export \"%s\"",
                        ...
                        if (!actual_export->extern_desc
                            ...
                            return member_name
                                       ? set_component_runtime_error_fmt(
                                             error_buf, error_buf_size,
                                             "component import \"%s\" instance "
                                             "export \"%s\" component export "
                                             "\"%s\" is missing component type "
                                             "metadata",
                                             import_name ? import_name
                                                         : "<unnamed>",
                                             member_name, expected_export_name)
                                       : set_component_runtime_error_fmt(
                                             error_buf, error_buf_size,
                                             "component import \"%s\" component "
                                             "export \"%s\" is missing component "
                                             "type metadata",
                                             import_name ? import_name
                                                         : "<unnamed>",
                                             expected_export_name);
                        ...
                        if (!resolve_component_component_type(
                                ...
                                &expected_nested_type, error_buf,
                                error_buf_size)
                            || !resolve_component_component_type(
                                actual_component,
                                actual_export->extern_desc->extern_desc.component
                                    .type_idx,
                                import_name, member_name, "actual",
                                &actual_nested_type, error_buf, error_buf_size)
                            || !validate_component_type_against_type(
                                expected_component, expected_nested_type,
                                actual_component, actual_nested_type,
                                import_name, member_name, remaining_depth - 1,
                                error_buf, error_buf_size))
                            return false;
                        break;
                    case WASM_COMP_EXTERN_TYPE:
                    ...
                    default:
                        return member_name
                                   ? set_component_runtime_error_fmt(
                                         error_buf, error_buf_size,
                                         "component import \"%s\" instance "
                                         "export \"%s\" uses unsupported typed "
                                         "component export member type 0x%02x",
                                         import_name ? import_name
                                                     : "<unnamed>",
                                         member_name,
                                         (unsigned)export_decl->extern_desc->type)
                                   : set_component_runtime_error_fmt(
                                         error_buf, error_buf_size,
                                         "component import \"%s\" uses "
                                         "unsupported typed component export "
                                         "member type 0x%02x",
                                         import_name ? import_name
                                                     : "<unnamed>",
                                         (unsigned)export_decl->extern_desc->type);
    ...
    for (i = 0; i < expected_type->count; i++) {
        ...
        switch (decl->tag) {
            ...
            case WASM_COMP_COMPONENT_DECL_EXPORT:
            {
                ...
                switch (expected_export_decl->extern_desc->type) {
                    case WASM_COMP_EXTERN_COMPONENT:
                        actual_export_decl = lookup_component_type_export_decl(
                            actual_type, expected_export_name,
                            WASM_COMP_EXTERN_COMPONENT);
                        ...
                        if (!resolve_component_component_type(
                            ...
                            || !resolve_component_component_type(
                                actual_component,
                                actual_export_decl->extern_desc->extern_desc
                                    .component.type_idx,
                                import_name, member_name, "actual",
                                &actual_nested_type, error_buf, error_buf_size)
                            || !validate_component_type_against_type(
    ...
    const char *member_name, char *error_buf, uint32 error_buf_size)
{
    if (!type_bound)
        return set_component_runtime_error_fmt(
            error_buf, error_buf_size,
            member_name ? "component import \"%s\" instance export \"%s\" is "
                          "missing a resource type bound"
                        : "component import \"%s\" is missing a resource type "
                          "bound",
            import_name ? import_name : "<unnamed>",
            member_name ? member_name : "");
    ...
    switch (type_bound->tag) {
        ...
        default:
            return set_component_runtime_error_fmt(
                error_buf, error_buf_size,
                member_name ? "component import \"%s\" instance export \"%s\" "
                              "uses unsupported resource type bound tag 0x%02x"
                            : "component import \"%s\" uses unsupported resource "
                              "type bound tag 0x%02x",
                import_name ? import_name : "<unnamed>",
                member_name ? member_name : "",
...
static bool
validate_component_runtime_resource_type_against_bound(
    const WASMComponentRuntimeResourceType *runtime_resource_type,
    const WASMComponentTypeBound *type_bound, const char *import_name,
    const char *member_name, char *error_buf, uint32 error_buf_size)
{
    if (!runtime_resource_type)
        return set_component_runtime_error_fmt(
            error_buf, error_buf_size,
            member_name ? "component import \"%s\" instance export \"%s\" is "
                          "missing resource type metadata"
                        : "component import \"%s\" is missing resource type "
                          "metadata",
            import_name ? import_name : "<unnamed>",
            member_name ? member_name : "");
            ...
            case WASM_COMP_COMPONENT_DECL_INSTANCE_EXPORTDECL:
            {
                ...
                switch (export_decl->extern_desc->type) {
                    ...
                    case WASM_COMP_EXTERN_TYPE:
                        if (!lookup_component_instance_export(
                                component_inst, member_name,
                                WASM_COMP_RUNTIME_REF_RESOURCE_TYPE, &ref,
                                error_buf, error_buf_size))
                            return false;
                        if (!validate_component_runtime_resource_type_against_bound(
                                ref.of.resource_type,
                                export_decl->extern_desc->extern_desc.type.type_bound,
                                import_name, member_name, error_buf,
                                error_buf_size))
                            return false;
                        break;
...
static bool
bind_top_level_imported_resource_type(WASMComponentInstance *inst,
                                      const WASMComponentImport *component_import,
                                      ...
                                      char *error_buf, uint32 error_buf_size)
{
    ...
    if (!binding->value.resource_type.drop_callback)
        return set_component_runtime_error_fmt(
            error_buf, error_buf_size,
            "component import binding \"%s\" is missing an imported resource "
            "drop callback",
    ...
    if (!inst->resource_state)
        return set_component_runtime_error_fmt(
            error_buf, error_buf_size,
            "component import \"%s\" could not resolve resource state",
            import_name ? import_name : "<unnamed>");

    for (i = 0; i < inst->resource_state->type_count; i++) {
        const WASMComponentRuntimeResourceType *resource_type =
            wasm_component_resource_lookup_runtime_type_const(inst->resource_state,
                                                             i);
        if (!resource_type
            || resource_type->kind != WASM_COMP_RUNTIME_RESOURCE_TYPE_IMPORTED
            || !resource_type->import_name || !import_name
    ...
    return set_component_runtime_error_fmt(
        error_buf, error_buf_size,
        "component import \"%s\" is not a bindable imported resource type",
        import_name ? import_name : "<unnamed>");
}
